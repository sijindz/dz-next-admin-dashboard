...
export function DataTableRowActions({ row }: { row: Access }) {
    const MySwal = withReactContent(Swal)
    const router = useRouter();
    async function deleteRow() {
        try {
            const response = await axiosInterceptorInstance.delete(`access/delete/${row?.id}`);
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[160px]">
                <DropdownMenuItem onClick={() => { router.push(`/access/edit/${row?.id}`) }}>
